pipeline {
    ...
    environment {
        DOCKER_HUB_REPO = 'nagarajunr2014/techsolutions-app'
    ...
    stages {
        stage('Checkout') {
            steps {
                ...
                git 'https://github.com/nagarajunr2014/microservices4-ingress.git'
            }
        }
        ...
        stage('Deploy to Kubernetes') {
            steps {
                ...
                script {
                    withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', credentialsId: 'aws-cred']]) {
                        sh "sed -i 's|nagarajunr2014/techsolutions-app:latest|kastrov/techsolutions-app:${env.IMAGE_TAG}|g' k8s/deployment.yaml"
